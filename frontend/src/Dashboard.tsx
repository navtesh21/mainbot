...
import { fetchDashboard, runScan, startBot, stopBot, fetchFootballSessions } from './api'
...
import { WhaleConsensusPanel } from './components/WhaleConsensusPanel'
...
export function Dashboard() {
  ...
  const { data: footballSessions = [] } = useQuery({
    queryKey: ['football-sessions'],
    queryFn: fetchFootballSessions,
    refetchInterval: 5000,
  })
  const activeFootballSession = footballSessions.find(s => s.status === 'running') ?? footballSessions[0] ?? null
    ...
    <div className="h-screen bg-black text-neutral-200 flex flex-col overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Link
            to="/crypto"
            className="px-1.5 py-0.5 text-[9px] font-bold uppercase border border-neutral-700 text-neutral-400 hover:text-neutral-200 hover:border-neutral-600 tracking-wider"
          >
            Crypto Scalp →
          </Link>
        </div>
      ...
      <div className="flex-1 min-h-0 grid grid-cols-[300px_1fr_340px] grid-rows-[1fr] gap-0">
        ...
        <div className="flex flex-col min-h-0 border-r border-neutral-800">
          {/* Football fixtures/live - top 58% */}
          <div className="relative min-h-0" style={{ height: '58%' }}>
            <FootballPanel />
          ...
          <div className="flex-1 min-h-0 grid grid-cols-2 border-t border-neutral-800">
            ...
            <div className="border-r border-neutral-800 flex flex-col min-h-0">
              <div className="px-2 py-1 border-b border-neutral-800 shrink-0">
                <span className="text-[10px] text-neutral-500 uppercase tracking-wider">AI Match Analysis</span>
              </div>
              <div className="flex-1 min-h-0 p-1">
                <MarketAnalysisPanel sessionId={activeFootballSession?.id ?? null} />
              </div>
            </div>
        ...
        <div className="flex flex-col min-h-0 overflow-hidden">
          {/* Whale Consensus - top portion */}
          <div className="flex flex-col min-h-0" style={{ height: '50%' }}>
            <div className="px-2 py-1 border-b border-neutral-800 flex items-center justify-between shrink-0">
              <span className="text-[10px] text-neutral-500 uppercase tracking-wider">Whale Consensus</span>
            </div>
