...
Ported verbatim from fball_bot/risk.py. Kept as a separate sizing/risk code
path from backend/core/signals.py's calculate_edge()/calculate_kelly_size()
per the design decision to reuse fball_bot's own tested risk stack wholesale
for football, rather than merge it with the BTC pipeline's simpler Kelly
formula. Only the persisted-state file path changed (now lives under
backend/football/_data/ instead of fball_bot's own data/ directory).
"""
...
class PositionRegistry:
    ...
    def __init__(self) -> None:
        ...
        self._fixture_buckets: dict[int, float] = {}
    ...
    def can_open(self, token_id: str, side: str, fixture_id: int, size_usd: float, capital: float) -> tuple[bool, str]:
        # NOTE: the per-token "only one open position at a time" policy here
        # is independently re-encoded in backend/football/session_manager.py's
        # _execute_entry() (its `if state.open_trades: return` gate). They
        # currently agree only because each session trades exactly one token —
        # keep both in sync if either policy ever changes.
    ...
    def open(self, token_id: str, side: str, fixture_id: int, size_usd: float) -> None:
        if token_id not in self._positions:
            self._positions[token_id] = []
        self._positions[token_id].append({"side": side, "fid": fixture_id, "size": size_usd})
        self._fixture_buckets[fixture_id] = self._fixture_buckets.get(fixture_id, 0.0) + size_usd

    def close(self, token_id: str, fixture_id: int, size_usd: float) -> None:
        pos_list = self._positions.get(token_id, [])
        self._positions[token_id] = [p for p in pos_list
                                      if not (p["fid"] == fixture_id and abs(p["size"] - size_usd) < 0.01)]
        self._fixture_buckets[fixture_id] = max(0.0, self._fixture_buckets.get(fixture_id, 0.0) - size_usd)
    ...
    def fixture_exposure(self, fixture_id: int) -> float:
        return self._fixture_buckets.get(fixture_id, 0.0)
...
class PortfolioRisk:
    ...
    def compute_max_trade_size(
        ...
        # base_size is the caller's own per-trade Kelly estimate (computed from
        # this specific trade's reward/risk ratio). Cap it by the portfolio's
        # own aggregate-history-derived Kelly capacity so a single optimistic
        # per-trade estimate can never exceed what the portfolio's learned
        # win/loss record actually supports — base_size was previously accepted
        # but silently discarded here, decoupling the cap from the caller's input.
    ...
    def _load_state(self) -> PortfolioState:
        try:
            ...
            logger.info(f"New football risk state: ${ic:.2f}")
...
# These three are process-wide singletons by design — risk limits/positions
# must be global across all concurrent football sessions, not per-match.
# This assumes a SINGLE worker process (true today: run.py's uvicorn.run()
# passes no `workers=`, so it defaults to 1). Do not deploy this app with
# `--workers N > 1` without first moving PortfolioRisk's state into a
# datastore all workers share — with N separate processes, each would build
# its own independent singleton and they would race on the same STATE_FILE.
...
def get_portfolio_risk() -> PortfolioRisk:
    ...
    if _portfolio_risk is None:
        ...
        _portfolio_risk = PortfolioRisk(
            max_per_trade_usd=settings.FOOTBALL_RISK_MAX_PER_TRADE_USD,
            initial_capital=settings.FOOTBALL_INITIAL_CAPITAL,
        )
...
def reset_singletons_for_tests() -> None:
    """Clear all three module-level singletons.

    Without this, any test importing this module inherits whatever state a
    prior test (or production run) left behind — including PortfolioRisk's
    on-disk STATE_FILE, which get_portfolio_risk() will happily reload. Call
    this in a test fixture's setup/teardown to make test order irrelevant.
    """
